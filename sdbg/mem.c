...
//reads memory
void read_memory(mach_port_t task, vm_address_t addr, vm_size_t bytes) {
    kern_return_t kret;
    
    printf(GOOD"Reading %d bytes from memory from address: 0x%lx\n", (int) bytes, addr);
    vm_address_t readOut;
    kret = vm_read_overwrite(task, addr, bytes, (vm_offset_t) &readOut, &bytes);

    if (kret == KERN_SUCCESS) {
        printf(GOOD"vm_read_overwrite result: 0x%lx - as int: %lu\n", readOut, readOut); }
...
//reads memory
void read_lines(mach_port_t task, char address[20], int lines, bool printchar) {
...
//see get_real_addr()
void read_offset(mach_port_t task, vm_address_t offset, vm_size_t bytes) {
    kern_return_t kret;
    
    printf(GOOD"Current slide is: 0x%lx\n", (vm_address_t) _dyld_get_image_vmaddr_slide(0));
    printf(GOOD"Reading %d bytes from memory from offset: 0x%lx\n", (int) bytes, get_real_addr(offset));
    vm_address_t readOut;
    kret = vm_read_overwrite(task, get_real_addr(offset), bytes, (vm_offset_t) &readOut, &bytes);

    if (kret == KERN_SUCCESS) {
        printf(GOOD"vm_read_overwrite result: 0x%lx - as int: %lu\n", readOut, readOut); }
    else if (kret != KERN_SUCCESS) {
        printf(GOOD"vm_read_overwrite (offset) failed: %s\n", mach_error_string(kret)); }

}

//write memory to address
void write_memory(mach_port_t task, vm_address_t addr, vm_address_t data) {
    kern_return_t kret;

    printf(GOOD"Writing memory: 0x%lx with data: 0x%lx\n", addr, data);
    kret = vm_write(task, addr, (vm_offset_t) &data, sizeof(data));
    
    if (kret == KERN_SUCCESS) {
        printf(GOOD"vm_write success at: 0x%lx\n", addr); }
    else if (kret != KERN_SUCCESS) {
...
void write_offset(mach_port_t task, vm_address_t offset, vm_address_t data) {
    kern_return_t kret;
    
    printf(GOOD"Current slide is: 0x%lx\n", (vm_address_t) _dyld_get_image_vmaddr_slide(0));
    printf(GOOD"Writing address: 0x%lx with data: 0x%lx\n", get_real_addr(offset), data);
    kret = vm_write(task, get_real_addr(offset), (vm_offset_t) &data, sizeof(data));
    
    if (kret == KERN_SUCCESS) {
        printf(GOOD"vm_write (offset) success at: %lx\n", get_real_addr(offset)); }
    else if (kret != KERN_SUCCESS) {
...
//vm_protect
void set_region_protection(mach_port_t task, vm_address_t addr, vm_size_t size) {
    kern_return_t kret;
    
    printf(GOOD"Setting %d byte region permissions at 0x%lx as R|W|X\n", (int) size, addr);
    kret = vm_protect(task, addr, size, 0, VM_PROT_READ | VM_PROT_WRITE | VM_PROT_EXECUTE);
        
    if (kret == KERN_SUCCESS) {
        printf(GOOD"vm_protect success!\n"); }
    else if (kret != KERN_SUCCESS) {
...
kern_return_t register_read(mach_port_t task, char reg[10]) {
    kern_return_t kret;
    ...
    if (strcmp(reg, "all\n") == 0) {
        printf(GOOD"Reading all registers...\n");
        for (int i = 0; i < 30; i++) {
...
kern_return_t register_write(mach_port_t task, char reg[10], uint64_t val) {
    kern_return_t kret;
